...
int	main(void)
{
	...
	fd_set			buffer_read_fds, buffer_write_fds;
	...
	FD_ZERO(&read_fds);
	FD_ZERO(&write_fds);
	FD_ZERO(&buffer_read_fds);
	FD_ZERO(&buffer_write_fds);
	...
	while (1)
	{
		read_fds = buffer_read_fds;
		write_fds = buffer_write_fds;
